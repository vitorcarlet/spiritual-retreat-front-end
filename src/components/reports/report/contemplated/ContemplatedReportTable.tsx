...
import { fetchContemplatedReport } from './shared';

interface ReportRow extends Record<string, unknown> {
  id: string | number;
}

const ContemplatedReportTable = ({ reportId }: { reportId: string }) => {
  ...
  const { data: reportData, isLoading } = useQuery({
    ...
    queryFn: () => fetchContemplatedReport(reportId, filters),
    staleTime: 5 * 60 * 1000,
  });
...
export default ContemplatedReportTable;
